...
function Post({ content, user, system, rotation = 0, timestamp, theme, reads = [] }: Omit<Post, 'id'> & { 
 theme: typeof themes[keyof typeof themes] 
}) {
...
function NewPostEditor({ onSubmit, user, theme }: { 
   ...
   <div className={`min-h-screen ${theme.bg} font-mono ${theme.text}`}>
     <div className="mx-auto max-w-2xl px-4 pt-4">
           ...
           {isNavExpanded && (
             <div className={`divide-y divide-dashed ${theme.border}`}>
               {tags.map(tag => (
                 <div
                   key={tag}
                   className={`p-3 ${tag === activeTag ? theme.accent : ''} 
                     cursor-pointer ${theme.accentHover}`}
       ...
       <div className="space-y-6 pb-16">
         ...
         <NewPostEditor onSubmit={createPost} user={user} theme={theme} />
